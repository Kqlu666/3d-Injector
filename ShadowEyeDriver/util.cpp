...
namespace Utils
{
    ...
    LDR_DATA_TABLE_ENTRY* ForEachKernelModule(bool (*callback)(LDR_DATA_TABLE_ENTRY*, void* parameter), void* parameter)
    {
        PLIST_ENTRY moduleList = (PLIST_ENTRY)PsLoadedModuleList;
    ...
    uint8_t* LoadFileIntoMemory(uint8_t** buffer, const wchar_t* FileName)
    {
        ...
        DbgPrint("filestandardinformation status %p, file_info.EndOfFile.QuadPart %p \n", status, file_info.EndOfFile.QuadPart);


        *buffer = (uint8_t*)ExAllocatePool(NonPagedPool, file_info.EndOfFile.QuadPart);

        auto status2 = ZwReadFile(hFile, NULL, NULL, NULL, &ioStatusBlock, *buffer, file_info.EndOfFile.QuadPart, &FileOffset, 0);
